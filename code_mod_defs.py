...
class ReplaceDefOrClass(cst.CSTTransformer):
    def __init__(self, target_name: str, new_code: str,
                 kind: Optional[str] = None, within_class: Optional[str] = None):
        """
        ...
        within_class: only replace when inside this class (for methods).
        """
    ...
    # Track current class nesting so we can target only a method in a specific class.
    ...
    def leave_ClassDef(self, original_node: cst.ClassDef, updated_node: cst.ClassDef) -> cst.BaseStatement:
        ...
        # Optionally replace a class definition itself:
        if (self.kind == "class"
            and original_node.name.value == self.target_name
            and (self.within_class is None)):  # replacing the class itself, not inside another
    ...
    def leave_FunctionDef(self, original_node: cst.FunctionDef, updated_node: cst.FunctionDef) -> cst.BaseStatement:
        ...
        if original_node.name.value != self.target_name:
            return updated_node
        if self.within_class is not None and self.within_class not in self.class_stack:
            return updated_node
        # Replace the function, preserving leading comments/blank lines
        return self.replacement.with_changes(leading_lines=original_node.leading_lines)
...
def replace_block(content: str,
                              ...
                              within_class: Optional[str] = None) -> str:
    """
    ...
    - If `within_class` is provided, only replace a method inside that class.
    """
    ...
    new_module = module.visit(ReplaceDefOrClass(target_name, new_code, kind=kind, within_class=within_class))
    return new_module.code



# Original modification functions
def replace_function_class(file_path, target_name, new_code):
    """Replace a function or class in a file with new code"""
    with open(file_path, 'r') as f:
        content = f.read()
    
    #new_content = replace_block(content, new_code, target_name)
    new_content = replace_block(content, new_code, target_name=target_name,)

    with open(file_path, 'w') as f:
        f.write(new_content)
    
    logging.debug(f"Replaced {target_name} in {file_path}")
...
def apply_modification_set(modifications, auto_rollback_on_failure=True):
    """
    Apply a set of modifications with comprehensive rollback support
